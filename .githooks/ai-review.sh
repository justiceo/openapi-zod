#!/usr/bin/env bash
# Reviews staged TypeScript/package.json/README.md changes against
...
reviewed_files="$(git diff --cached --name-only -- '*.ts' '*.tsx' 'package.json' 'README.md' ':!test/fixtures/**')"
if [ -z "$reviewed_files" ]; then
  echo "[ai-review] No staged TypeScript, package.json, or README.md changes; skipping (fixtures excluded)."
  exit 0
fi

diff="$(git diff --cached -- '*.ts' '*.tsx' 'package.json' 'README.md' ':!test/fixtures/**')"
...
diff may include package.json and README.md alongside TypeScript files;
apply the 'General change-level guides' and dependency rules to those.
