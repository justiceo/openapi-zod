import { MAX_SCHEMA_DEPTH } from "./core.js";
...
export function convertSchema(schema, context) {
    if (context.depth.current >= MAX_SCHEMA_DEPTH) {
        addDiagnostic(context, "invalid.schema", `Schema nesting exceeds the maximum supported depth (${MAX_SCHEMA_DEPTH}); likely a circular or pathological schema.`, context.path);
        return "z.unknown()";
    }
...
function convertSchemaInner(schema, context) {
...
function errorMessageFor(schema, keyword) {
...
const IDN_HOSTNAME_REGEX = "/^[\\p{L}\\p{N}](?:[\\p{L}\\p{N}-]{0,61}[\\p{L}\\p{N}])?(?:\\.[\\p{L}\\p{N}](?:[\\p{L}\\p{N}-]{0,61}[\\p{L}\\p{N}])?)*$/u";
...
function convertString(schema, context) {
    ...
    if (customFormat) {
        context.customFormatsUsed.add(schema.format);
...
function formatOptionsArgument(schema, context) {
...
function integerBase(schema) {
    return schema.format === "int32" ? "z.int32()" : "z.int()";
}
function numberBase(schema) {
...
function convertArray(schema, context) {
    if (Array.isArray(schema.prefixItems)) {
        ...
        let expression = rest === undefined ? `z.tuple([${tupleItems.join(", ")}])` : `z.tuple([${tupleItems.join(", ")}], ${rest})`;
...
function schemaHasDefault(schema, context) {
    const object = asRecord(schema);
    if (!object) {
        return false;
    }
    ...
    if (typeof object.$ref === "string" && object.$ref.startsWith("#/components/schemas/")) {
        ...
        return isSchemaObject(targetSchema) && targetSchema.default !== undefined;
    }
    return false;
}
...
function computeStronglyConnectedComponents(graph) {
    ...
    for (const start of graph.keys()) {
        ...
        const work = [
            { node: start, iterator: (graph.get(start) ?? new Set()).values() },
        ];
        ...
        while (work.length > 0) {
            ...
            if (!next.done) {
                ...
                if (!index.has(child)) {
                    ...
                    stack.push(child);
                    onStack.add(child);
                    work.push({ node: child, iterator: (graph.get(child) ?? new Set()).values() });
                }
