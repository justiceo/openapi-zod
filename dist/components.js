...
import { asRecord, escapePointer, literalObjectExpression, objectExpression, propertyKey, sanitizeIdentifier, unescapePointer, uniqueName, zodObjectExpression, } from "./emit.js";
...
function convertHeader(header, _name, path, shared) {
...
function defaultResponseExpression(shared) {
    if (!shared.options.includeDefaultValues) {
        return "{}";
    }
    return '{ description: "", headers: z.object({}), content: {} }';
}
function convertSecurityScheme(scheme, path, shared) {
...
function parseReusableRef(value, kind, path, shared) {
...
function reusableRefName(value, kind, path, shared) {
    ...
    const exportName = kind === "parameters"
