...
import { searchOperation } from "./operations.js";
...
export async function search(config: ClientConfig, input: ClientOperationInput<typeof searchOperation.request> = {}, options?: ClientOptions): Promise<ClientResult<ClientResponseData<typeof searchOperation.responses>>> {
  return clientRequest(config, searchOperation, input, options);
}

export function createClient(config: ClientConfig) {
  return {
    search: (input?: ClientOperationInput<typeof searchOperation.request>, options?: ClientOptions) => search(config, input, options),
  };
}
