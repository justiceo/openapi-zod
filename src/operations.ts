import { type ConvertedParameter, convertParameter, convertRequestBody, convertResponse } from "./components.js";
  ...
  for (const pathKey of Object.keys(paths).sort()) {
    if (!pathKey.startsWith("/")) {
      shared.diagnostics.push(
        diagnostic("invalid.path", "Path keys must start with /.", `#/paths/${escapePointer(pathKey)}`, shared.options),
      );
      continue;
    }
    ...
    if (pathItem.$ref !== undefined) {
      shared.diagnostics.push(
        diagnostic(
          ...
          "Path item $ref is not supported.",
          `#/paths/${escapePointer(pathKey)}/$ref`,
          shared.options,
        ),
      );
    }
...
function convertOperationRequest(
  ...
  if (Array.isArray(pathItem.parameters)) {
    pathItem.parameters.forEach((parameter, index) =>
      addParameter(parameter, `${operationPath}/../../parameters/${index}`),
    );
  }
  ...
  for (const parameter of Array.from(merged.values()).sort((a, b) =>
    ...
    if (!isResponseStatus(key)) {
      shared.diagnostics.push(
        diagnostic(
          ...
          `Invalid response status "${key}".`,
          `${operationPath}/responses/${escapePointer(key)}`,
          shared.options,
        ),
      );
      continue;
...
function validatePathParameters(
  ...
  for (const name of templateNames) {
    if (params[name] === undefined) {
      shared.diagnostics.push(
        diagnostic("invalid.pathParameter", `Missing path parameter "${name}".`, operationPath, shared.options),
      );
    }
  }
    ...
    if (!templateNames.includes(name)) {
      shared.diagnostics.push(
        diagnostic(
          ...
          `Path parameter "${name}" is not used in the path template.`,
          operationPath,
          shared.options,
        ),
      );
...
function validateSecurityRequirements(
  ...
  if (!Array.isArray(security)) {
    shared.diagnostics.push(
      diagnostic("invalid.securityScheme", "Security requirements must be an array.", path, shared.options),
    );
    return;
  }
    ...
    for (const name of Object.keys(object)) {
      if (!shared.securityNames.has(name)) {
        shared.diagnostics.push(
          diagnostic(
            "invalid.securityScheme",
            `Security scheme "${name}" was not found.`,
            `${path}/${index}/${escapePointer(name)}`,
            shared.options,
          ),
        );
        continue;
      ...
      if (!Array.isArray(scopes)) {
        shared.diagnostics.push(
          diagnostic(
            ...
            `Security scopes for "${name}" must be an array.`,
            `${path}/${index}/${escapePointer(name)}`,
            shared.options,
          ),
        );
        ...
        for (const [scopeIndex, scope] of scopes.entries()) {
          if (typeof scope !== "string" || !knownScopes.has(scope)) {
            shared.diagnostics.push(
              diagnostic(
                "invalid.securityScope",
                `Security scope "${String(scope)}" was not declared.`,
                `${path}/${index}/${escapePointer(name)}/${scopeIndex}`,
                shared.options,
              ),
            );
          }
...
function derivedOperationId(method: HttpMethod, pathKey: string, shared: SharedContext, path: string): string {
  shared.diagnostics.push(
    diagnostic(
      ...
      "Operation is missing operationId; deriving a deterministic name.",
      path,
      shared.options,
    ),
  );
