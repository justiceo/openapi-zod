...
export function convertClientFunctions(
  ...
  for (const { fnName, exportName } of entries) {
    lines.push("");
    lines.push(
      `export async function ${fnName}(config: ClientConfig, input: ClientOperationInput<typeof ${exportName}.request> = {}, options?: ClientOptions): Promise<ClientResult<ClientResponseData<typeof ${exportName}.responses>>> {`,
    );
  ...
  for (const { fnName, exportName } of entries) {
    lines.push(
      `    ${fnName}: (input?: ClientOperationInput<typeof ${exportName}.request>, options?: ClientOptions) => ${fnName}(config, input, options),`,
    );
  }
  lines.push("  };");
  lines.push("}");
