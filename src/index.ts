...
function convertReusableComponents(documentObject: Record<string, unknown> | undefined, shared: SharedContext): ReusableResult {
  ...
  for (const name of Object.keys(headers).sort()) {
    ...
    result.lines.push("", `export const ${exportName} = ${convertHeader(headers[name], `${exportName}`, `#/components/headers/${escapePointer(name)}`, shared)};`);
  }
  ...
  for (const name of Object.keys(parameters).sort()) {
    ...
    result.lines.push("", `export const ${exportName} = ${convertParameter(parameters[name], `#/components/parameters/${escapePointer(name)}`, shared).schema};`);
  }
  ...
  for (const name of Object.keys(requestBodies).sort()) {
    ...
    result.lines.push("", `export const ${exportName} = ${convertRequestBody(requestBodies[name], `#/components/requestBodies/${escapePointer(name)}`, shared)};`);
  }
  ...
  for (const name of Object.keys(responses).sort()) {
    ...
    result.lines.push("", `export const ${exportName} = ${convertResponse(responses[name], `#/components/responses/${escapePointer(name)}`, shared)};`);
  }
  ...
  for (const name of Object.keys(securitySchemes).sort()) {
    ...
    if (shared.options.includeSecurityValidators) {
      result.lines.push("", `export const ${exportName} = ${convertSecurityScheme(securitySchemes[name], `#/components/securitySchemes/${escapePointer(name)}`, shared)};`);
    }
  }
...
function convertParameter(parameter: unknown, path: string, shared: SharedContext): ConvertedParameter {
  ...
  return { name: key, location: location ?? "query", schema, required };
}
...
function convertOperations(documentObject: Record<string, unknown> | undefined, shared: SharedContext & { securityNames: Map<string, string> }): OperationsResult {
  ...
  for (const pathKey of Object.keys(paths).sort()) {
    ...
    for (const method of httpMethods) {
      ...
      lines.push("");
      lines.push(`export const ${exportName} = ${objectExpression({
...
function convertSchema(schema: unknown, context: ConvertContext): string {
  ...
  if (object.prefixItems !== undefined) {
    addDiagnostic(context, "unsupported.tuple", "Tuple schemas are not supported.", `${context.path}/prefixItems`);
  }

  if (object.uniqueItems === true) {
    addDiagnostic(context, "unsupported.uniqueItems", "uniqueItems is not supported.", `${context.path}/uniqueItems`);
  }
  ...
  if (Object.prototype.hasOwnProperty.call(object, "const")) {
    return literalExpression(object.const, context, "unsafe.literal") ?? "z.unknown()";
  }

  if (Array.isArray(object.enum)) {
    return convertEnum(object.enum, context);
  }
...
function convertRef(ref: string, schema: Record<string, unknown>, context: ConvertContext): string {
  ...
  let expression =
    context.cycles.has(edge) || targetOrder > currentOrder
      ? `z.lazy(() => ${targetName})`
      : targetName;
